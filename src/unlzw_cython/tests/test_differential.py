...
def _fuzz_inputs():
    ...
    for n in (50, 200, 511, 512, 513, 1023, 1024, 1025, 2047, 2048, 2049, 3000, 20000):
...
def test_differential_fuzz(tmp_path: Path, max_bits: int) -> None:
    for i, data in enumerate(_fuzz_inputs()):
        ...
        subprocess.run(["compress", "-f", "-b", str(max_bits), str(raw)], check=True, capture_output=True)
...
class TestInvalidInput:
    ...
    def test_too_short(self, fun):
        with pytest.raises(ValueError, match="too short"):
            fun(b"\x1f")

    def test_bad_magic_bytes(self, fun):
        with pytest.raises(ValueError, match="magic bytes"):
            fun(b"\x00\x00\x80")

    def test_reserved_flag_bits_set(self, fun):
        with pytest.raises(ValueError, match="invalid data"):
            fun(bytes([0x1F, 0x9D, 0x60]))

    def test_max_bits_out_of_range(self, fun):
        with pytest.raises(ValueError, match="out of range"):
            fun(bytes([0x1F, 0x9D, 0x80 | 17]))

    def test_partial_code(self, fun):
        with pytest.raises(ValueError, match="middle of a code"):
            fun(bytes([0x1F, 0x9D, 0x80 | 9, 0x00]))

    def test_empty_stream_is_valid(self, fun):
